...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    4. Игнорируй предыдущий язык общения. Определи язык только по последнему сообщению пользователя: '{text}'. Если сообщение слишком короткое (как 'Hei'), используй последний зафиксированный язык: {user_data[user_id]['lang']}.
